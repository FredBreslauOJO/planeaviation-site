...
(function () {
  ...
  function navHTML(){
    return navLinks.map(l =>
      `<a href="${l.href}" class="${isActive(l.href).trim()}" data-i18n="${l.key}">${l.key}</a>`
    ).join("");
  }

  /* ---- SELETOR DE IDIOMA ---- */
  function langSelector(cls=""){
    return `<div class="lang-selector ${cls}">
  ...
  <header class="site">
    <div class="container">
      <div class="hd-row">
        <a href="index.html" class="logo" aria-label="Plane Aviation">
          ${LOGO_DARK}
        </a>
        <nav class="nav">${navHTML()}</nav>
        <div class="hd-actions">
          ${langSelector()}
          <a href="contato.html" class="btn btn-solid btn-xs" data-i18n="nav_cta">nav_cta</a>
        </div>
    ...
    <div class="mobile-nav" id="mobileNav">
      <nav>
        ${navHTML()}
        ${langSelector("mobile")}
        <a href="contato.html" class="btn btn-solid btn-sm" data-i18n="nav_cta">nav_cta</a>
      </nav>
    </div>
  </header>`;
  ...
  <footer class="site">
    <div class="container">
      <div class="f-grid">
        <div>
          <div class="f-logo" style="margin-bottom:1rem">
            <a href="index.html" aria-label="Plane Aviation">${LOGO_LIGHT}</a>
          </div>
          <div class="f-selo"><span data-i18n="f_partner">f_partner</span></div>
          <p class="f-desc" data-i18n="f_desc">f_desc</p>
        </div>
        <div class="f-col">
          <h4 data-i18n="f_nav">f_nav</h4>
          <nav>
            ${footerLinks.map(l=>`<a href="${l.href}" data-i18n="${l.key}">${l.key}</a>`).join("")}
            <a href="contato.html" class="last" data-i18n="f_consultant">f_consultant</a>
          </nav>
        </div>
        <div class="f-col f-contact">
          <h4 data-i18n="f_contact">f_contact</h4>
        ...
        <div class="f-col f-news">
          <h4 data-i18n="f_newsletter">f_newsletter</h4>
          <p data-i18n="f_newsletter_p">f_newsletter_p</p>
          <div class="f-news-row">
            <input type="email" data-i18n="f_newsletter_ph" placeholder="Seu e-mail">
            <button data-i18n="f_newsletter_btn">f_newsletter_btn</button>
          </div>
        </div>
      </div>
      <div class="f-bottom">
        <p data-i18n="f_copyright">f_copyright</p>
        ${langSelector("footer")}
      </div>
    </div>
  </footer>`;
